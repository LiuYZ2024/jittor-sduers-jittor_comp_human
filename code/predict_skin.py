...
def predict(args):
    ...
    for batch_idx, data in tqdm(enumerate(predict_loader)):
        ...
        for i in range(B):
            ...
            if _id > 10000:
                # 直接复用 id-10000 的 predict_skin
                ref_id = _id - 10000
                ref_path = os.path.join(predict_output_dir, cls[i], str(ref_id), "predict_skin.npy")
                skin_resampled = np.load(ref_path)
...
def main():
    ...
    parser.add_argument('--num_sample', type=int, default=16,
                        help='Batch size for training')
    parser.add_argument('--vertices_sample', type=int, default=16,
                        help='Batch size for training')
