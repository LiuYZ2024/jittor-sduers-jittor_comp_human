...
from models.skin_uniseg import create_model
...
def train(args):
    ...
    train_loader = get_dataloader(
        ...
        shuffle=True,
        sampler=SamplerMix(num_samples=args.num_sample, vertex_samples=args.vertices_sample, use_hand_bias=False),
        transform=transform,
    ...
    for epoch in range(args.epochs):
        ...
        if val_loader is not None and (epoch + 1) % args.val_freq == 0:
            ...
            for batch_idx, data in enumerate(val_loader):
                ...
                if batch_idx == show_id:
                    ...
                    for i in id_to_name:
                        ...
                        exporter._render_skin(path=f"tmp_{args.output_dir}/skin/epoch_{epoch}/{name}_ref.png",vertices=vertices.numpy()[0], skin=skin.numpy()[0, :, i], joint=joints[0, i])
                        exporter._render_skin(path=f"tmp_{args.output_dir}/skin/epoch_{epoch}/{name}_pred.png",vertices=vertices.numpy()[0], skin=outputs.numpy()[0, :, i], joint=joints[0, i])
            ...
            if val_loss_l1 < best_loss:
                ...
                model_path = os.path.join(args.output_dir, f'best_model_{epoch+1}.pkl')
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default='pct',
                        choices=['pct', 'pct2', 'custom_pct', 'skeleton','uniseg'],
    ...
    parser.add_argument('--num_sample', type=int, default=2048,
                        help='number of sampled points from the point cloud')
    parser.add_argument('--vertices_sample', type=int, default=1024,
                        help='number of sampled vertices from the mesh')
