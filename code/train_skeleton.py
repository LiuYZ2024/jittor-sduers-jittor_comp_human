        ...
        if val_loader is not None and (epoch + 1) % args.val_freq == 0:
            ...
            for batch_idx, data in enumerate(val_loader):
                ...
                if batch_idx == show_id:
                    ...
                    exporter._render_skeleton(path=f"tmp_{args.output_dir}/skeleton/epoch_{epoch}/skeleton_ref.png", joints=joints[0].numpy().reshape(-1, 3), parents=parents)
                    exporter._render_skeleton(path=f"tmp_{args.output_dir}/skeleton/epoch_{epoch}/skeleton_pred.png", joints=outputs[0].numpy().reshape(-1, 3), parents=parents)
                    exporter._render_pc(path=f"tmp_{args.output_dir}/skeleton/epoch_{epoch}/vertices.png", vertices=vertices[0].permute(1, 0).numpy())
...
def main():
    ...
    parser.add_argument('--num_sample', type=int, default=1024,
                        help='fu')
    parser.add_argument('--vertices_sample', type=int, default=512,
                        help='c')
